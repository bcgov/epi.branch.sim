...
* `sim_params` is a named list containing the global parameters that will affect all cases. These include the disease parameters, simulation control parameters, and the definition of the distributions used to draw case-specific parameters. The contents of this list are constant within a simulation.

* `sim_status` is a named list acting as the "state vector" containing bookkeeping quantities not specific to any case that is generally updated at every timestep of the simulation.
