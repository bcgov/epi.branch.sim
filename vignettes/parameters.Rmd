...
### Disease parameters (distributions): incubation period and serial intervals
...
#### Serial intervals
In this simulation, the serial interval is defined to be the number of days between start of infection of the index case to the start of infection of a potential secondary case. The serial interval distribution is provided as the `serial_int_params` argument, which expects a named list. The list member `$dist` sets the probability distribution function to use. Valid values for `dist` are `skew_norm` and `gamma`. Each of these choices take shape and scale parameters from additional list members with names that match shape and scale argument names of the corresponding R function that generates random numbers from the given distribution. The parameters should be given so that the distribution function generates random serial interval values in days.

| Serial interval distribution | List members required | Description |
| :--------------------------- | :-------------------- | :---------- |
| Skew normal                  | `$dist='skew_norm'`   | Uses `sn::rsn` to draw numbers. The `xi` location parameter is equal to the incubation period. This distribution is used to parameterize how often serial intervals are shorter than the incubation period (i.e. pre-symptomatic transmission). |
...
##### Examples
The serial interval distribution from [Bi et al. (2020)](https://www.medrxiv.org/content/10.1101/2020.03.03.20028423v3):
```{r serial_int_example_bi}
...
The benchmark serial interval distribution from [Hellewell et al. (2020)](https://doi.org/10.1016/S2214-109X(20)30074-7), where 15% of serial intervals are shorter than the incubation period:
```{r serial_int_example_hellewell}
serial_int_params <- list(
  dist='skew_norm', 
  omega=2, 
  alpha=1.95
...
2. Serial intervals are drawn for each potential secondary infection from the serial interval distribution.

3. Serial intervals that fall outside the time frame where the index case could infect others are prevented from happening.
    + If the index case is an imported case, it may be following self-quarantine guidelines so serial intervals shorter than the length of this quarantine period are prevented from becoming secondary infections.
    + If the index case is eventually isolated, serial intervals that happen after isolation time are prevented from becoming secondary infections.
    + Serial intervals that happen after the length of infection (`infect_dur`) are also prevented from becoming secondary infections.
