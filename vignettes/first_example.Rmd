...
Now we define parameters for the disease spread, interventions and other simulation control parameters. The following represent one example of these parameters and do not cover the entire range of options.
...
sd_params <- list(
  ...
  sd_contact_rate1 = 1.0, # initial fraction of contacts for distancing group 
...
```
Run `?draw_sd_factor` for more information on the above choices. All contact rates are given with respect to the non-distancing group, which has a contact rate of 1.
...
```
Run `?generate_imported_infections` to see the other stochastic and deterministic options for the number of daily imported cases.
...
head(state_df)
```
Note that all initial cases are considered to be untraced. At the start of the simulation, the `sd_factor` for all cases is 1 because the distancing measures are delayed until day 7. Note that all times in `state_df` are measured in a relative sense: most times are measured from the start of infection, except for `isolation_delay`, which is measured from the symptom onset (`incubation_length`). The `serial_intervals` column stores the times where the case would cause a secondary infection. Here, we see that case 1 will cause an infection at time 0.73, i.e. by the next time-step. The column `non_infect_serials` lists potential secondary infections that were prevented due to that case's isolation. See `create_state_df` for more information on these columns.

Now we can take a peek at `record_df`.
```{r peek record_df}
head(record_df)
```
This data frame stores information that was also available in the `state_df` data frame (e.g. `case_id`, `is_traced`, etc.) but provides additional columns for better tracking. While information is removed from `state_df` when it is no longer needed (for instance, the serial intervals list shortens as the infections are loaded into the simulation and rows are removed entirely when the case no longer has further pending infections), `record_df` retains all the information until the end of the simulation. Here, columns for milestones beginning with `d_` (e.g. `d_incub`) are durations measured as a relative time, in days. On the other hand, columns such as `t_symp` are absolute simulation times which have value `NA` until these milestones occur. Note that not all cases will reach all milestones. See `?create_record_df` for more information on how these columns are populated with information.
...
It's important to note that `t_inf` (and other `t_*` columns) are given at the discrete simulation time-steps only (i.e. multiples of `dt`, which is 1 in this example). Remember that the first serial interval of Case 1 was 0.73 days, which led to Case 4. For the purposes of propagating infections and calculating Case 4's isolation time, Case 4 was infected at time `t=0.73 days`. But in these columns, `t_inf` for Case 4 is on day 1. Thus, these columns indicate the time-step where the cases reach these milestones, not the actual time. These columns are best used for debugging rather than analysis.
...
tail(state_df, n=5)
```
These are the most recent five cases ending with Case #301. The negative value of `isolation_delay` in Case #300 indicates that this case will be isolated before the end of its incubation period. This happens because Case #300 is traced by the app which causes traced cases to be isolated as soon as the index case is isolated, leading to quick isolations. The value of 0.3 in the `sd_factor` column in three of the last five cases indicates that these cases are physically distancing and only have 0.3 times as many contacts as they normally would. 
